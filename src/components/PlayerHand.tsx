...
const PlayerHand: React.FC = () => {
  ...
  const handleCompletePlayCards = () => {
    ...
    // 获取选中的卡牌信息
    const selectedCards = player.cards.filter(card => selectedIds.includes(card.id))
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-md">
      ...
      {/* 法术牌操作栏 */}
      {!isPlayCardsMode && selectedIds.length > 0 && player.cards.find((c: any) => c.id === selectedIds[0] && c.type === 'spell') && (
        <button
