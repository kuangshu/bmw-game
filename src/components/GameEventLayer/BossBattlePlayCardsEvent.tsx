...
// BOSS战斗出牌事件数据类型
interface BossBattlePlayCardsEventData {
  // 可以添加特定于BOSS战斗出牌事件的属性
}

interface BossBattlePlayCardsEventProps {
  eventData: GameEventData<BossBattlePlayCardsEventData>;
  onComplete: (result: { playedCards: Card[] }) => void;
}

const BossBattlePlayCardsEvent: React.FC<BossBattlePlayCardsEventProps> = ({ onComplete }) => {
  ...
  const handlePlayCards = () => {
    const playedCards = selectedCards.map(cardId => player?.getCard(cardId)).filter(Boolean) as Card[];
